...
/**
 * Implementation of WebScraperService for the first website.
 */
@Service
public class Website1ScraperService implements WebScraperService {
    
    private static final Logger logger = LoggerFactory.getLogger(Website1ScraperService.class);
    private static final String WEBSITE_NAME = "Website1";
    ...
    public List<MachineryItem> scrapePage(String url) {
        ...
        try {
            ...
            Elements itemElements = doc.select(".machinery-item");
            ...
            for (Element itemElement : itemElements) {
                ...
                item.setModel(extractText(itemElement, ".model"));
                item.setContractType(extractText(itemElement, ".contract-type"));
                item.setMake(extractText(itemElement, ".make"));
                item.setYear(extractText(itemElement, ".year"));
                item.setWorkedHours(extractText(itemElement, ".hours"));
                item.setCity(extractText(itemElement, ".city"));
                item.setPrice(extractText(itemElement, ".price"));
                item.setPhotoUrl(extractAttribute(itemElement, ".photo img", "src"));
